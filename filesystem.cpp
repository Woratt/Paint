...
void FileSystem::saveImage(QPixmap* canvas, const QString& nameImage){
    canvas->toImage().save(nameImage, "PNG");
}

void FileSystem::undo(Canvas* canvas){
    canvas->takeImageWithHistory(false);
}

void FileSystem::redo(Canvas* canvas){
    canvas->takeImageWithHistory(true);
}
